...
{
    int ret = 0;
    ret = rs232_rs422_power_set(UW_DEVICE_GPS_PORT, SERIAL_POWER_ON);
    if (ret)
    {
    ...
    ret = rs232_rs422_mode_switch(UW_DEVICE_GPS_PORT, SERIAL_RS232_MODE);
    if (ret)
    {
    ...
    ret = rs232_rs422_cfg(UW_DEVICE_GPS_PORT, 19200, 8, 1, 0);
    if (ret)
    {
        printf("Config error...");
        return false;
...
{
    int ret = rs232_rs422_send(UW_DEVICE_GPS_PORT, buf, len);
    if (ret != len)
    {
        printf("Send error...%d\n", ret);
    }
...
int uw_gps_recv(char *buf, int len)
{
    int ret = rs232_rs422_recv(UW_DEVICE_GPS_PORT, buf, len);
...
{
    int ret = uw_gps_recv(p_read_char, sizeof(char));
    if (ret != sizeof(char))
    {
...
{
    char read_byte;
    int ret = rs232_rs422_recv(UW_DEVICE_GPS_PORT, &read_byte, 1);
...
void* gps_processing(void* arg)
{
    ...
    {
        bool read_ret = false;
        read_char = uw_gps_read_byte(&read_ret);
        if (!read_ret)
        {
